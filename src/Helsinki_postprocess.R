...
LIBRARIES <- c(#"clusterProfiler", 
               "ggrepel", "ggplot2", "reshape2", 
               "gridExtra", "gtable", "ggpubr", "ggVennDiagram",
               "assertthat", "parallel", "dplyr", "MASS")
...
DIR <- "/Users/vandu/Work/BEAtDKD_analysis"
ADJUST.METHOD <- 'BH'
...
PROJECT <- "Helsinki3" #"BristolHelsinki" #"Helsinki"
setwd(file.path(DIR, "output", "Helsinki"))
OMICS <- switch(PROJECT,
                Bristol = c("miRNAseq"),
                c("RNAseq", "Prot", "miRNAseq"))
CELLS <- switch(PROJECT,
                Helsinki3 = c("ABN", "K29", "PT34"),
                c("ABN", "GEnC", "K29", "PT34"))
CELLS.display <- switch(PROJECT,
                        Helsinki3 = setNames(c("Pod", "MC", "PTC"), CELLS),
                        setNames(c("Pod", "GEC", "MC", "PTC"), CELLS))

CONDS <- switch(PROJECT,
                c("Basal", "InsulinResistant", "IR_Basal", "IR_InsulinResistant"))
COLORGROUPS <- list(BEAtDKDcell=setNames(c("coral", "gray40", "turquoise3", "gold2"),
                                         c("ABN", "GEnC", "K29", "PT34")),
                    BEAtDKDcond=setNames(c("green2", "magenta2", "darkgreen", "darkmagenta"), 
                                         CONDS))
SHAPEGROUPS <- list(BEAtDKDcell=setNames(c(16, 15, 18, 17), c("ABN", "GEnC", "K29", "PT34")),
                    BEAtDKDcond=setNames(c(18, 15, 16, 17), CONDS))
...
dirs.all <- paste0(OMICS, ifelse(PROJECT=="Helsinki3", "no_GEnCIR", "allIR"))
##-----
...
##- link Bristol and Helsinki DE results
if (PROJECT=="BristolHelsinki") {
    design.Helsinki <- read.delim(file.path(DIR, "Helsinki_design.cfg"), header=T)
    
    scatterplot.all <- invisible(lapply(dirs, function(x) {
        Pval <- titlex <- if (grepl("^Prot", x)) 0.05 else
            if (grepl("^RNAseq", x)) 0.05 else
                if (grepl("^miRNAseq", x)) 0.05 else
                    ""
        tab1 <- read.table(file.path(DIR, "output/Bristol", x, 'DE.tsv'), header=T, sep='\t', quote="", row.names=1)
        tab2 <- read.table(file.path(DIR, "output/Helsinki", x, 'DE.tsv'), header=T, sep='\t', quote="", row.names=1)
        #tab1 <- tab1[tab1[, grep("^p_value", colnames(tab1))] < Pval,]
        #tab2 <- tab2[tab2[, grep("^p_value", colnames(tab2))] < Pval,]
        features.common <- intersect(sub("\\.(\\d)+", "", rownames(tab1)), rownames(tab2))
        log2FC.df <- cbind.data.frame("Cell"=tab1[features.common, grep("^Log2FC", colnames(tab1))],
                                      "EV"=tab2[features.common, grep("^Log2FC", colnames(tab2))],
                                      "name"=features.common)
        
        cellx <- gsub("(^RNAseq|^Prot|^miRNAseq|IR$)", "", x)
        titlex <- if (grepl("^Prot", x)) "Proteomics" else
            if (grepl("^RNAseq", x)) "RNA-seq" else
                if (grepl("^miRNAseq", x)) "miRNA-seq" else
                    ""
        
        p <- ggplot(log2FC.df, aes(x=Cell, y=EV)) +
            geom_point() +
            geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
            ggtitle(paste0(titlex, " ", CELLS.display[cellx], "_IR")) +
            xlab("log2FC in Cell") +
            ylab("log2FC in EV") +
            theme_bw() +
            theme(
    ...
    ag <- ggarrange(plotlist=scatterplot.all,
                    common.legend=T, ncol=3, nrow=4)
    ggsave(ag, filename="Fig_scatterplot_FC1.pdf", width=12, height=16)
}
...
omics.pca.plots.all <- lapply(dirs.all, function(x) {
    #tab <- read.table(paste0(x, '/1_DE.tsv'), header=T, sep='\t', quote="", row.names=1)
    tab <- log2(read.table(paste0(x, '/normalized.tsv'), header=T, sep='\t', quote="", row.names=1))
    counts <- tab[,!grepl("^FDR_|^p_value|^UpSetGroup|^name|^Log2FC_|^pKruskal", colnames(tab))]
    ...
    pca.df$cell <- rownames(pca.df) %>% gsub("_.+", "", .)
    pca.df$group <- rownames(pca.df) %>% 
        gsub("_ABN.+|_K29.+|_GEnC.+|_PT34.+|_GTG.+", "", .) %>%
        gsub("_PD.+|_MC.+|_GC.+|_PT.+", "", .) %>%
        gsub("ABN_|K29_|GEnC_|PT34_", "", .) %>%
        gsub("^_", "", .) %>%
        gsub("_[0-9]+$", "", .)
    pca.df$color <- COLORGROUPS$BEAtDKDcell[pca.df$cell]
    pca.df$cell <- pca.df$cell %>% CELLS.display[.] 
    # for all cell types
    ...
    pca.plots.all <- lapply(1L:length(pca.dims), function(i) {
        ...
        p <- pca.plot +
            ...
            geom_point(aes(x=!!pc1,
                           y=!!pc2,
                           colour=cell, shape=group), size=4) +
            xlab(paste0("PC",
                        pca.dims[[i]][1],
                        " (",
            ...
            scale_colour_manual(name="Cell",
                                values=unname(COLORGROUPS$BEAtDKDcell[names(sort(CELLS.display))]),
                                guide=guide_legend(title.position='top')) +
            scale_shape_manual(name="Condition",
                               values=SHAPEGROUPS$BEAtDKDcond[as.vector(unique(pca.df$group))],
                               guide=guide_legend(title.position='top')) +
...
ag <- ggarrange(plotlist=c(unlist(omics.pca.plots.all, recursive = F))[c(1,3,5,2,4,6)], 
                # labels=c("a", "b", rep("", 2)),
                # font.label=list(size=40, face='bold'),
                # label.y=1.02,
                common.legend=T, ncol=3, nrow=2)
ggsave(ag, filename="Fig_PCA3.pdf", width=18, height=12)
...
omics.pca.plots.each <- lapply(dirs.all, function(x) {
    ...
    pca.plots.each <- lapply(names(sort(CELLS.display)), function(cel) {
        ...
        pca.df$batch <- if (grepl("^Prot", x)) proteomics.info[rownames(pca.df), 'batch'] else 16
        ...
        pca.plots <- lapply(1L:length(pca.dims), function(i) {
            ...
            if (grepl("^Prot", x))
                p <- p + 
                    geom_point(aes(x=PC1,
                                   y=PC2,
                                   colour=group, shape=batch), size=5)
            else           
                p <- p + 
                    geom_point(aes(x=PC1,
                                   y=PC2,
...
ag <- ggarrange(plotlist=unlist(unlist(omics.pca.plots.each, recursive = F), recursive = F),
                ...
                ncol=4, nrow=3)
ggsave(ag, filename="Fig_PCA_cell.pdf", width=20, height=15)
##-----

##- number of DE genes per contrast ----
# for all cell types in one model
regulated.df <- do.call(rbind, lapply(dirs.all, function(x) {
    tab <- read.table(paste0(x, '/1_DE.tsv'), header=T, sep='\t', quote="")
    const <- sub(grep("^FDR_", colnames(tab), value=T), pattern='^FDR_', replacement='')
    df <- melt(setNames(lapply(c(0, log2(FC)), function(lfc) {
        lfc.df <- t(sapply(const, function(cnt) {
            fc.cnt <- paste0("Log2FC_", cnt)
            #fdr.cnt <- paste0("FDR_", cnt)
            p.cnt <- paste0("p_value_", cnt)
            c(Down=sum(tab[, fc.cnt] < -lfc & tab[, p.cnt] < PVal),
              Up  =sum(tab[, fc.cnt] >  lfc & tab[, p.cnt] < PVal))
        }))
...
invisible(assert_that(!any(paste0(regulated.df[regulated.df$Cutoff==paste0("FC > ", FC), ]$Contrast,
                                  regulated.df[regulated.df$Cutoff==paste0("FC > ", FC), ]$Regulation)!=
                               paste0(regulated.df[regulated.df$Cutoff=="FC > 1", ]$Contrast,
                                      regulated.df[regulated.df$Cutoff=="FC > 1", ]$Regulation)),
                      msg="Error in melt!"))
regulated.df$Stack <- regulated.df$Size * (regulated.df$Cutoff==paste0("FC > ", FC))
regulated.df[regulated.df$Cutoff=="FC > 1",]$Stack <- regulated.df[regulated.df$Cutoff=="FC > 1",]$Size - 
    regulated.df[regulated.df$Cutoff==paste0("FC > ", FC),]$Size

regulated.df$Cell <- regulated.df$Contrast %>% gsub(".+_in_", "", .) %>% gsub("_.+", "", .)
regulated.df$IR <- ifelse(grepl("IR_|_IR", regulated.df$Contrast), "IR", "")
regulated.df$Omics <- regulated.df$Contrast %>% gsub("allall_.+|IR_.+|_.+", "", .) %>%
    factor(., levels=c("RNAseq", "Prot", "miRNAseq"))
...
colors <- setNames(c("#92C5DE", "#F4A582", "#0571B0", "#CA0020"),
                   apply(expand.grid(c("Down.FC > ", "Up.FC > "), as.character(c(1, FC))), 1, paste, collapse="")
                   )
...
regulated.df$Color <- factor(ifelse(
    ...
    ifelse(regulated.df$Cutoff=="FC > 1", colors["Down.FC > 1"], colors[paste0("Down.FC >", FC)]),
    ifelse(regulated.df$Cutoff=="FC > 1", colors["Up.FC > 1"], colors[paste0("Up.FC > ", FC)])
), levels=colors)
...
names(colors) <- names(colors) %>% 
    sub("Down.FC > 1$", "FC < 1", .) %>%
    sub(paste0("Down.FC > ", FC), paste0("FC < ", fractions(1/FC)), .) %>%
    sub("Up.FC >", "FC >", .)
# plot
p <- ggplot(regulated.df, aes(x=Cond, y=Stack, fill=interaction(Regulation, Cutoff))) +
    facet_wrap(~Omics, ncol=3, scales='free',
               labeller=labeller(Omics=setNames(c("Number of DE proteins", "Number of DE RNAs", "Number of DE miRNAs"), 
                                                c("Prot", "RNAseq", "miRNAseq")))) +
    xlab("") + ylab("") +
...
ggsave(p, filename='Fig_DE.pdf', width=18, height=6.1)
##-----
...
volcano.list <- unlist(setNames(mclapply(dirs.all, mc.cores=length(dirs.all), function(x) {
    ...
    setNames(lapply(const, function(cnt) {
        ...
        mat <- cbind(paste0(gsub(x, pattern=paste(CELLS, collapse='|'), replacement=''), '_', cnt), 
                     tab[, c('name', fc.cnt, p.cnt)])
        rownames(mat) <- rownames(tab)
        colnames(mat) <- c("Contrast", "ID", "Log2FC", "Pval")
        if (grepl("^miRNA", x))
            mat[,"ID"] <- sub("hsa-miR-", "", rownames(mat))
        return (mat)
    }), const)
...
volcano.df$sig <- ifelse(volcano.df$Pval < Pval,
                         ifelse(volcano.df$Log2FC > log2(FC), 
                                colors[paste0("FC > ", FC)],
                                ifelse(volcano.df$Log2FC < -log2(FC), 
                                       colors[paste0("FC < ", fractions(1/FC))],
                                       ifelse(volcano.df$Log2FC < 0, 
                                              colors["FC < 1"],
                                              colors["FC > 1"]))),
                         "grey70")

# volcano plot for each omics
sapply(OMICS, function(omic) {
    volcano.df.plot <- volcano.df %>% subset(., IR=="IR" & Omics==omic)
    volcano.df.plot$sig <- factor(volcano.df.plot$sig, levels=unique(volcano.df.plot$sig))
    annotate.df <- volcano.df.plot[volcano.df.plot$sig %in% c("#0571B0", "#CA0020"), c("Cond", "sig")] %>%
        group_by(Cond, sig) %>%
        reframe(n=n()) %>%
        as.data.frame()
    labelx <- quantile(volcano.df.plot$Log2FC, ifelse(omic=="RNAseq", 0.9999, 0.9992))
    annotate.df <- cbind.data.frame(annotate.df, t(apply(annotate.df, 1, function(adr) {
        vdp <- volcano.df.plot[volcano.df.plot$Cond==adr["Cond"],]
        return (c(x=ifelse(unname(adr["sig"])=="#CA0020", labelx, -labelx),
                  y=0,
                  hjust=ifelse(unname(adr["sig"])=="#CA0020", 0, 1),
                  label=paste0("(", sum(vdp$sig==adr["sig"]), " DE ",
                               switch(omic, 'miRNAseq'='miRNAs', 'RNAseq'='RNAs', 'Prot'='proteins'),
                               ")")))
    })))
    annotate.df$x <- as.numeric(annotate.df$x)
    annotate.df$y <- as.numeric(annotate.df$y)
    p <- ggplot(volcano.df.plot, 
                aes(x=Log2FC, y=-log10(Pval), colour=sig)) +
        geom_point(size=1, alpha=1, na.rm=T) +
        geom_text_repel(data=subset(volcano.df.plot, 
                                    sqrt(Log2FC^2 + (-log10(Pval))^2) > quantile(sqrt(Log2FC^2 + (-log10(Pval))^2), ifelse(omic=="miRNAseq", 0.9, 0.9)) &
                                        Pval<0.01),
                        aes(x=Log2FC, y=-log10(Pval), label=ID), box.padding = 0.5, max.overlaps = 20, size=6) +
        scale_color_manual(name="", values=levels(volcano.df.plot$sig), guide='none') +
        geom_hline(yintercept = -log10(Pval), colour="darkgreen", linetype="dashed") +
        geom_vline(xintercept =  log2(FC), colour="darkgreen", linetype="dashed") +
        geom_vline(xintercept = -log2(FC), colour="darkgreen", linetype="dashed") +
        annotate("text", label=paste0("FC = ", FC),
                 x=log2(FC), y=max(-log10(volcano.df.plot$Pval))*1.1,
                 hjust=-0.1, vjust=-0.3, color="darkgreen", size=6) + 
        annotate("text", label=paste0("FC = ", fractions(1/FC)),
                 x=-log2(FC), y=max(-log10(volcano.df.plot$Pval))*1.1,
                 hjust=1.1, vjust=-0.3, color="darkgreen", size=6) +
        geom_text(data=annotate.df, aes(x=x, y=y, label=label, color=sig, hjust=hjust),
        ...
        ylim(0, max(-log10(volcano.df.plot$Pval))*1.1) +
        theme_bw() +
        theme(title           = element_text(size=12, face='bold'),
              legend.text     = element_text(size=16),
              legend.position = "top",
    ...
    ggsave(p, file=paste0('Fig_volcano_',omic,'.pdf'), width=12, height=12)
})
##-----
